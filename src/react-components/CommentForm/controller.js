...
if ( CausalityRedux.store[CommentBox_Partition].getState().items.length === 0 ) {
    ...
    initialComments.forEach(comment => CausalityRedux.store[CommentBox_Partition].onAddComment(comment) );
}
