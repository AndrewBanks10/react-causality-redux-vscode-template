...
const monitorComponentPartition = 'monitorComponentPartition'
...
function setThisState () {
  ...
  allStates[currentState].store[monitorComponentPartition] = getState()
...
function onStateChange (arg) {
  if (arg.partitionName !== monitorComponentPartition && monitorMirroredState.isDebugging) {
    setTimeout(stopDebug, 1)
  }
  if (arg.partitionName !== monitorComponentPartition && arg.partitionName !== CausalityRedux.storeHistoryKey && arg.operation !== CausalityRedux.operations.STATE_FUNCTION_CALL) {
...
function onListener (arg) {
  if (arg.partitionName !== monitorComponentPartition && arg.operation !== CausalityRedux.operations.STATE_FUNCTION_CALL) {
    listeners.push(arg)
  }
}
